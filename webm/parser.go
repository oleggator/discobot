...
func Parse(r io.ReadSeeker, m *WebM) (*Reader, error) {
	...
	err = e.Unmarshal(m)
	if dt := m.Header.DocType; dt != "webm" && dt != "webm\000" && dt != "matroska" {
		return nil, errors.New("not a WebM or matroska file")
	}
	...
	if err != nil && err.Error() == "Reached payload" {
		...
		if pos := m.cuesPosition(); pos > 0 {
			curr, _ := segment.Seek(0, io.SeekCurrent)
			segment.Seek(pos+sh.Offset, io.SeekStart)
			ce, _ := segment.Next()
			ce.Unmarshal(&m.Segment.Cues)
			segment.Seek(curr, io.SeekStart)
		}
